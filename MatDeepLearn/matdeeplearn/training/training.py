...
def evaluate_loss(model, data_loader, loss_method):
    model.eval()
    loss_all = 0
    count = 0
    with torch.no_grad():
...
def train(model, optimizer, loader, loss_method, reg, reg_funct, rank):
    model.train()
    loss_all = 0
    count = 0
    ...
    for data in loader:
        ...
        reg_lambda = reg
        if reg_funct == 'L1':
            norm = sum(p.abs().sum() for p in model.parameters()) #L1
        elif reg_funct == 'L2':
            norm = sum(p.pow(2.0).sum() for p in model.parameters()) #L2
        else:
            raise ValueError("Invalid regularization function. Must be either 'L1' or 'L2'.")
     
        loss = loss + torch.tensor(reg_lambda) * norm
        ...
        with torch.no_grad():
            ...
            if out == True:
                if count == 0:
                    ...
                    ids = ids + ids_temp
                    predict = np.concatenate((predict, output.data.cpu().numpy()), axis=0)
...
def trainer(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Train model
        train_error = train(model, optimizer, train_loader, loss, reg, reg_funct, rank=rank)
        if rank not in ("cpu", "cuda"):
        ...
        if val_loader is not None and rank in (0, "cpu", "cuda"):
            if rank not in ("cpu", "cuda"):
                val_error = evaluate_loss(model.module, val_loader, loss)
            else:
                val_error = evaluate_loss(model, val_loader, loss)
        ...
        if val_loader is not None and rank in (0, "cpu", "cuda"):
            if val_error == float("NaN") or val_error < best_val_error:
                ...
                early_stopping(val_error)  # Check for early stopping

            if early_stopping.early_stop:
                print("Early stopping triggered. Stopping training...")
                break

        elif val_loader is None and rank in (0, "cpu", "cuda"):
            if rank not in ("cpu", "cuda"):
                model_best = copy.deepcopy(model.module)
                torch.save(
                    {
...
def train_regular(
    ...
    ##Get dataset
    dataset = process.get_dataset(data_path, training_parameters["target_index"], features, False)
    ...
    model = trainer(
        ...
        model_parameters["reg"],
        model_parameters["reg_funct"],
...
def train_CV(
    ...
    ##Get dataset
    dataset = process.get_dataset(data_path, training_parameters["target_index"], "False", False)
...
def tune_trainable(config, checkpoint_dir=None, data_path=None):
    ...
    for epoch in range(1, model_parameters["epochs"] + 1):
        lr = scheduler.optimizer.param_groups[0]["lr"]
        train_error = train(
            model, optimizer, train_loader, training_parameters["loss"], model_parameters["reg"], model_parameters["reg_funct"], rank=rank
        )
