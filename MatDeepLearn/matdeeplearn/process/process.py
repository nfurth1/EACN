...
def split_data_CV(dataset, num_folds=5, seed=np.random.randint(1, 1e6), save=False):
    ...
    cv_dataset = torch.utils.data.random_split(dataset, folds, generator=torch.Generator().manual_seed(seed))
...
def get_dataset(data_path, target_index, features, reprocess="False", processing_args=None):
    if processing_args == None: processed_path = "processed"
    else: processed_path = processing_args.get("processed_path", "processed")
    transforms = GetY(index=target_index)
    if os.path.exists(data_path) == False:
        print("Data not found in:", data_path)
        sys.exit()
    if reprocess == "True":
        os.system("rm -rf " + os.path.join(data_path, processed_path, features))
    ...
    if os.path.exists(os.path.join(data_path, processed_path, "data.pt")) == True:
        ...
        process_data(data_path, processed_path, processing_args, features)
        if os.path.exists(os.path.join(data_path, processed_path, "data.pt")) == True:
            dataset = StructureDataset(data_path, processed_path,  transforms,)
        elif os.path.exists(os.path.join(data_path, processed_path, "data0.pt")) == True:
            dataset = StructureDataset_large(data_path, processed_path,  transforms,)
...
def process_data(data_path, processed_path, processing_args, features):
    ...
    ##Load dictionary (Loads FilePath)
    ...
    ##Load targets (Makes Array of Targets/GroundTruths)
    ...
    for index in range(0, len(target_data)):
        ...
        if processing_args["data_format"] != "db": #(NA)
    ...
    for index in range(0, len(data_list)):
        data_list[index] = OneHotDegree(
            data_list[index], data_path, index, processing_args["graph_max_neighbors"] + 1, features
        )
...
def OneHotDegree(data, data_path, index, max_degree, features, in_degree=False, cat=True):
    ...
    if x is not None and cat:
        ...
        data.x = deg
    if features == "True":
        file = str(data_path) + '\\values.csv'
        
        vals = pd.read_csv(file, names = ['Mols','vals'])
        vals = vals.iloc[:,1:vals.shape[0]]
        sz = data.x.size()
        print(data.x)
        
        c = nn.ConstantPad2d((0, 1, 0, 1), 0)
        data.x = c(data.x) 
        
        for i in range(vals.shape[1]):
            data.x[i, sz[1]] = vals.iloc[index, i]
    return data
